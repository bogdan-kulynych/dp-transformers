...
def main_preprocess_function(
    ...
    for i in range(batch_size):
        ...
        model_inputs["input_ids"][i] = torch.tensor(
            model_inputs["input_ids"][i][:sequence_len]
        )
        model_inputs["attention_mask"][i] = torch.tensor(
            model_inputs["attention_mask"][i][:sequence_len]
        )
...
class Dataset:
    ...
    prompt_begin = (
        None  # Prompt to add to the beginning of the text, e.g. "Sentence : "
    )
    ...
    def preprocess_function(self, example):
        return main_preprocess_function(
            example,
            self.tokenizer,
            self.text_field,
            self.prompt_begin,
    ...
    def __init__(self, tokenizer, sequence_len):
        ...
        self.dataset = self.dataset.map(
            lambda x: {
                "text_concat": [
                    question + " ### " + sentence
                    for question, sentence in zip(x["question"], x["sentence"])
                ]
            },
            batched=True,
            num_proc=8,
        ...
        self.dataset["validation"] = (
            self.dataset["validation"].shuffle().select(range(1000))
        )
...
class CNNDataset(Dataset):
    ...
    def compute_test_metrics(self, trainer):
        ...
        test_dataset = test_dataset.map(
            lambda x: {
                "article": [
                    self.prompt_begin + article + self.prompt_end
        ...
        responses = [
            trainer.tokenizer.decode(r.squeeze(), skip_special_tokens=True)
        ...
        result = self.evaluate.compute(
            ...
            references=test_dataset["summary"][
                trainer.args.device.index :: trainer.args.world_size
            ],
            use_stemmer=True,
        )

        r1 = trainer.accelerator.reduce(
            torch.tensor(result["rouge1"]).to(trainer.args.device), reduction="mean"
        )
        r2 = trainer.accelerator.reduce(
            torch.tensor(result["rouge2"]).to(trainer.args.device), reduction="mean"
        )
        rl = trainer.accelerator.reduce(
            torch.tensor(result["rougeL"]).to(trainer.args.device), reduction="mean"
        )
